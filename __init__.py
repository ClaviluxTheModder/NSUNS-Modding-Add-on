...
def sn_append_panel_F7D8E(self,context):
    try:
        layout = self.layout
        op = layout.operator("export_scene.xfbin",text=r"Export XFBIN",emboss=True,depress=False,icon_value=707)
        op.filepath = r""
        ...
        op.meshes_to_export = []
        split = layout.split(align=False,factor=0.8225806355476379)
        split.enabled = True
        split.alert = False
        split.scale_x = 1.0
        split.scale_y = 1.0
